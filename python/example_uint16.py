#!/usr/bin/env python3
"""
Example driver for lsCOMP uint16 compressor using ctypes + pycuda.

Usage (from project root):

  cd python
  python example_uint16.py \
      -i ../data/input_uint16.bin \
      -d 128 128 64 \
      -b 4 8 16 32 \
      -p 0.01 \
      -x cmp_uint16.bin \
...
Options:
   -i oriFilePath: Path to the original data file (uint16 raw binary)
...
def parse_args():
    parser = argparse.ArgumentParser(description="lsCOMP uint16 Python example")

    parser.add_argument(
        "-i", "--input",
        dest="ori_path",
        required=True,
        help="Path to original uint16 raw data file",
    )
    parser.add_argument(
        "-d", "--dims",
        nargs=3,
...
def main():
    ...
    cmp_size = compressor.compress_uint16(
        d_ori_ptr=int(d_ori),
        d_cmp_ptr=int(d_cmp),
        dims=dims,
    ...
    max_abs_diff = np.max(
        np.abs(
            host_dec.astype(np.int32) - host_data.astype(np.int32)
        )
    )

    print("=== lsCOMP uint16 example ===")
